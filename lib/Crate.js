...
Crate.prototype.plugin = function(schema, options) {
	...
	schema.post("init", function() {
		...
		collections[model._id.toString()] = {};
		...
		Object.keys(options.fields).forEach(function(field) {
			...
			collections[model._id.toString()][field] = model[field];
		}.bind(this));
	});
	...
	schema.pre("save", function (next) {
		...
		if(!collections[model._id.toString()]) {
			return next();
		}

		Object.keys(collections[model._id.toString()]).forEach(function(field) {
			collections[model._id.toString()][field].forEach(function(oldDoc) {
				var present = false;
