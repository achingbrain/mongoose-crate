...
Crate.prototype.plugin = function(schema, options) {
  ...
  schema.methods.attach = function(field, attachment, callback) {
    ...
    // build suggested target storage location
    var format = options.fields[field].url || options.url || ':modelName/:fieldName/:id'
    tasks.push(function(callback) {
      targetBuilder(this, field, attachment, format, function (error, target) {
