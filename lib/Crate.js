...
Crate.prototype.plugin = function(schema, options) {
  ...
  schema.methods.attach = function(field, attachment, callback) {
    ...
    // remove the old file if we're about to overwrite it
  ...
  schema.virtual('__cached_attachments').set(function(status) {
    return this.___cached_attachments = status
  })
  ...
  schema.post('init', function() {
    ...
    Object.keys(options.fields).forEach(function(field) {
      this.__cached_attachments[field] = model[field]
    }.bind(this))
  })
  ...
  schema.pre('save', function (next) {
    ...
    Object.keys(options.fields).forEach(function(field) {
      if(options.fields[field].array) {
        ...
        // deal with attachments that have been removed from fields
        if(!model[field].url && model.__cached_attachments[field] && model.__cached_attachments[field].url) {
          tasks.push(function (callback) {
